...
Integration: late fusion at head input, \(z=[z_{\text{cnn}};L]\), following the idea of explicit geometric supervision through landmarks \cite{bulat2017far}.
...
Integration: \(z_{\text{hm}}\) is concatenated into the classifier head vector, consistent with heatmap-based keypoint representations \cite{newell2016stacked}.
...
Integration: \(\tilde{I}\) replaces the original image in the backbone stream; \(z_{\text{att}}\) is appended at head fusion, analogous to lightweight channel-spatial attention designs \cite{woo2018cbam}.
...
Integration: \(z_{\text{sobel}}\) is fused by concatenation at the classifier head \cite{sobel1968isotropic}.
...
Integration: \(z_{\text{gnn}}\) is concatenated with backbone and other branch embeddings, following message-passing and graph-convolution principles \cite{gilmer2017neural,kipf2017semi}.
